...
object ArticleRepository {
    ...
    fun sendComment(articleId: String, comment: String, answerToSlug: String?) {
        network.sendMessage(
            ...
            User("777", "Jonh Doe", "https://skill-branch.ru/img/mail/bot/android-category.png")
        )
...
class CommentsDataSource(
    ...
    override fun loadInitial(
        ...
        Log.e(
            "ArticleRepository",
            "loadInitial: key > ${params.requestedInitialKey} size > ${params.requestedLoadSize} totalCount > ${result.size}"
        )
        callback.onResult(
    ...
    override fun loadAfter(params: LoadParams<String>, callback: LoadCallback<CommentItemData>) {
        ...
        Log.e(
            "ArticleRepository",
            "loadAfter: key > ${params.key} size ${result.size}"
        )
    ...
    override fun loadBefore(params: LoadParams<String>, callback: LoadCallback<CommentItemData>) {
        ...
        Log.e(
            "ArticleRepository",
            "loadBefore: key > ${params.key} size ${result.size}"
        )
        callback.onResult(result)
    }
